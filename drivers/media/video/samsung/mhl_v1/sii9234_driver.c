...
//#define TX_DEBUG_PRINT2(x,y...) printk(x,##y);
#define TX_DEBUG_PRINT2(x,y...) do {} while(0)
...
Bool SiI9234_startTPI(void)
{
 ...
 devID = ReadByteTPI(TPI_DEVICE_ID);
  ...
  {
    TX_DEBUG_PRINT2("######## Silicon Device Id: %x\n", devID);
    return TRUE;
  }

    TX_DEBUG_PRINT2("Unsupported TX\n");
    return FALSE;
}
...
Bool SiI9234_init(void)
{
  ...
  TX_DEBUG_PRINT2("# SiI9234 initialization start~ \n");  
	...
	FSA9480_MhlSwitchSel(0);
         ...
         else
        {
          TX_DEBUG_PRINT((KERN_ERR "RSEN Stable\n"));
			...
			{
			mhl_cable_status =MHL_TV_OFF_CABLE_CONNECT;
			TX_DEBUG_PRINT2(KERN_ERR "MHL Connection Fail Power off ###3\n");
			SiI9234_HW_Reset();
			For_check_resen_int();
			...
			return ;
	      		...
	      		{
			TX_DEBUG_PRINT2(KERN_ERR "%s: MHL Cable disconnect### 2\n", __func__);
			mhl_cable_status =MHL_INIT_POWER_OFF;
  ...
  if(reg71 & BIT_6)
  {
	...
	//
  	...
  	if((BIT_6 & cbusInt) != dsHpdStatus)
  	{
			...
			#if 0 //subhransu fix for cbus interrupts
			if(rsen == 0x00)
			{
				I2C_WriteByte(SA_TX_CBUS_Primary, 0x09, 0x00);
				I2C_WriteByte(SA_TX_CBUS_Primary, 0x1F, 0x00);
				mhl_cable_status = MHL_TV_OFF_CABLE_CONNECT;
			}
#else
...
static	void	MhlTxDriveStates( void )
{
	...
	switch( mhlTxConfig.mscState )
	{
      ...
      TX_DEBUG_PRINT2("MSC_STATE_BEGIN \n");
		...
		case MSC_STATE_POW_DONE:
			...
			//
			TX_DEBUG_PRINT2("MSC_STATE_POW_DONE \n");
...
//
// It is called in interrupt context to meet some MHL specified timings, therefore,
// it should not have to call app layer and do negligible processing, no TX_DEBUG_PRINT2s.
//
void	SiiMhlTxMscCommandDone( byte data1 )
{
...
void	SiiMhlTxNotifyDsHpdChange( byte dsHpdStatus )
{
	if( 0 == dsHpdStatus )
	{
	    printk(("MhlTx: Disable TMDS###\n"));
		SiiMhlTxDrvTmdsControl( FALSE );
	}
	else
	{
	    printk(("MhlTx: Enable TMDS####\n"));
		SiiMhlTxDrvTmdsControl( TRUE );
	}
}
///////////////////////////////////////////////////////////////////////////////
...
//	TX_DEBUG_PRINT2("App: Got event = %02X, eventParameter = %02X\n", (int)event, (int)eventParameter);

	switch( event )
	{
		case	MHL_TX_EVENT_DISCONNECTION:
			TX_DEBUG_PRINT2("App: Got event = MHL_TX_EVENT_DISCONNECTION\n");
			break;

		case	MHL_TX_EVENT_CONNECTION:
			TX_DEBUG_PRINT2("App: Got event = MHL_TX_EVENT_CONNECTION\n");
			break;
		...
		case	MHL_TX_EVENT_RCP_READY:
			...
			TX_DEBUG_PRINT2("App: Got event = MHL_TX_EVENT_RCP_READY...Sending RCP (%02X)\n", (int) rcpKeyCode);

      #if 0
			if( (0 == (MHL_FEATURE_RCP_SUPPORT & eventParameter)) )
			{
				TX_DEBUG_PRINT2( "App: Peer does NOT support RCP\n" );
			}
			if( (0 == (MHL_FEATURE_RAP_SUPPORT & eventParameter)) )
			{
				TX_DEBUG_PRINT2( "App: Peer does NOT support RAP\n" );
			}
			if( (0 == (MHL_FEATURE_SP_SUPPORT & eventParameter)) )
			{
				TX_DEBUG_PRINT2( "App: Peer does NOT support WRITE_BURST\n" );
			}
			...
			{
				TX_DEBUG_PRINT2("App: SiiMhlTxRcpSend (%02X)\n", (int) rcpKeyCode);
			}
			else
			{
				TX_DEBUG_PRINT2("App: SiiMhlTxRcpSend (%02X) Returned Failure.\n", (int) rcpKeyCode);
			}
      #endif
		...
		case	MHL_TX_EVENT_RCP_RECEIVED:
			...
			//
			TX_DEBUG_PRINT2("App: Received an RCP key code = %02X\n", eventParameter );
...
rcpKeyCode = eventParameter;
		...
		case	MHL_TX_EVENT_RCPK_RECEIVED:
			TX_DEBUG_PRINT2("App: Received an RCPK = %02X\n");
			break;

		case	MHL_TX_EVENT_RCPE_RECEIVED:
			TX_DEBUG_PRINT2("App: Received an RCPE = %02X\n");
			break;
...
void	AppVbusControl( bool powerOn )
{
	if( powerOn )
	{
		TX_DEBUG_PRINT2("App: Peer's POW bit is set. Turn the VBUS power OFF here.\n");
	}
	else
	{
		TX_DEBUG_PRINT2("App: Peer's POW bit is cleared. Turn the VBUS power ON here.\n");
		I2C_WriteByte(SA_TX_CBUS_Primary, 0x09, 0x00);
		I2C_WriteByte(SA_TX_CBUS_Primary, 0x1F, 0x00);
		mhl_cable_status = MHL_TV_OFF_CABLE_CONNECT;
	}
