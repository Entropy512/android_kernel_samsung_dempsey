...
void mxt224_ta_probe(int ta_status)
{
	u16 obj_address = 0;
	u16 size_one;
	int ret;
	...
	/*u8 calcfg;*/
	u8 noise_threshold,noise_crtl, cte_actvdepth;
	u8 movfilter;
	u8 calcfg_dis;
	u8 calcfg_en;
	u16 i;
	...
	if (ta_status) {
		threshold = 60;
		threshold_e = 40;
		noise_threshold = 25;//40;
		noise_crtl = 135;
		movfilter = 46;
		cte_actvdepth = 63;
		
		//110926
		  if( noise_freq_table.t9_blen_for_fherr_cnt > 0)
		  {
		   threshold = 40;
		   noise_threshold =35;
		   movfilter = 0;
	...
	if (copy_data->family_id==0x81) {
		...
		{	
			if (noise_freq_table.fherr_setting) {
				...
				size_one = 1;
				value  = 16;
				write_mem(copy_data, obj_address+6, size_one, &value);

				ret = get_object_info(copy_data, PROCG_NOISESUPPRESSION_T22, &size_one, &obj_address);
				size_one = 1;				
				for (i = 0; i < 5; i++) {
					value = noise_freq_table.freq_for_fherr4[i];
					write_mem(copy_data, obj_address+11+i, size_one, &value);
				}
				

				noise_freq_table.fherr_count = 0;
				noise_freq_table.fherr_setting = false;
#if 1 // Eplus Andy Add 20110922			
				noise_freq_table.t9_blen_for_fherr_cnt = 0;
				printk("[TSP] t9_blen_for_fherr_cnt init !!!! \n");
#endif
			}
		}
		ret = get_object_info(copy_data, TOUCH_MULTITOUCHSCREEN_T9, &size_one, &obj_address);
		...
		ret = get_object_info(copy_data, SPT_CTECONFIG_T28, &size_one, &obj_address);
		value = cte_actvdepth;
		register_address = 4;
		size_one = 1;
		write_mem(copy_data, obj_address+(u16)register_address, size_one, &value);
		read_mem(copy_data, obj_address+(u16)register_address, (u8)size_one, &val);
		printk(KERN_ERR"[TSP]TA_probe MXT224 T%d Byte%d is %d\n", 28, register_address, val);
		
	}
};
EXPORT_SYMBOL(mxt224_ta_probe);
...
static int __devinit mxt224_init_touch_driver(struct mxt224_data *data)
{
	...
	if (ret)
	{
		printk("mxt224_init_touch_driver read_mem fail = %d\n", ret);
		return ret;
	}
...
static int freq_hop_err_setting(int state)
{
	...
	ret = get_object_info(copy_data, GEN_POWERCONFIG_T7, &size, &obj_address);
	size = 1;
	value = (u8)255; //idleacqint
	write_mem(copy_data, obj_address, size, &value);

	ret = get_object_info(copy_data, TOUCH_MULTITOUCHSCREEN_T9, &size, &obj_address);
	value = noise_freq_table.t9_movfilter_for_fherr;
	write_mem(copy_data, obj_address+13, 1, &value);


	value = noise_freq_table.t9_thr_for_fherr;
	write_mem(copy_data, obj_address+7, 1, &value);


	value = noise_freq_table.t9_blen_for_fherr;
	write_mem(copy_data, obj_address+6, 1, &value);


	ret = get_object_info(copy_data, PROCG_NOISESUPPRESSION_T22, &size, &obj_address);
	value = noise_freq_table.t22_noisethr_for_fherr;
	write_mem(copy_data, obj_address+8, 1, &value);

	ret = get_object_info(copy_data, SPT_CTECONFIG_T28, &size, &obj_address);
	value = noise_freq_table.t28_cte_for_fherr;
	write_mem(copy_data, obj_address+4, 1, &value);


	
#if 0 // Eplus Andy Add 20110922			

	if(noise_freq_table.t9_blen_for_fherr_cnt== 1)
	{
		ret = get_object_info(copy_data, TOUCH_MULTITOUCHSCREEN_T9, &size, &obj_address);
		value = noise_freq_table.t9_blen_for_fherr; 
		write_mem(copy_data, obj_address+6, 1, &value);
		...
		value = noise_freq_table.t22_noisethr_for_fherr;
		write_mem(copy_data, obj_address+8, 1, &value);
		printk("[TSP] t9_blen_for_fherr_cnt == 1 \n");
	}
#endif
	...
	for (i = 0; i< 5; i++) {
		if (state == 1) {
			value = noise_freq_table.freq_for_fherr1[i];	
			write_mem(copy_data, obj_address+11+i, 1, &value);
		} else if (state == 2) {
			value = noise_freq_table.freq_for_fherr2[i];
			write_mem(copy_data, obj_address+11+i, 1, &value);
		} else if (state == 3) {
			value = noise_freq_table.freq_for_fherr3[i];
			write_mem(copy_data, obj_address+11+i, 1, &value);
		} else if (state == 4) {
			value = noise_freq_table.freq_for_fherr4[i];
			write_mem(copy_data, obj_address+11+i, 1, &value);
...
//		printk(KERN_ERR"[TSP] Starting irq with 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x \n", msg[0], msg[1], msg[2], msg[3], msg[4], msg[5], msg[6], msg[7]);
		...
		if ((msg[0] == 15) && (data->family_id==0x80)&&((msg[1]&0x08) == 0x08)) { /* noise suppression */
		//	printk(KERN_ERR"[TSP] Starting irq with 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x", msg[0], msg[1], msg[2], msg[3], msg[4], msg[5], msg[6], msg[7]);
			if (is_cable_attached) {
				...
				if (noise_freq_table.fherr_count > 12)
				{
					...
					if(noise_freq_table.t9_blen_for_fherr_cnt < 1)
					{
						noise_freq_table.t9_blen_for_fherr_cnt = 1;
					}
#endif
				}
				if (!(noise_freq_table.fherr_count%3))
				{
...
#if 1 // eplus_Andy modifed 110909		
...
#ifdef QT_FIRMUP_ENABLE
{
	...
	if (data->family_id == 0x80) {	/*  : MXT-224 */
		...
		noise_freq_table.t9_blen_for_fherr_cnt = 0; 
		noise_freq_table.t9_thr_for_fherr = 30;
		noise_freq_table.t9_movfilter_for_fherr = 0;
		noise_freq_table.t22_noisethr_for_fherr = 20;
		noise_freq_table.t28_cte_for_fherr = 63;
		
		noise_freq_table.freq_for_fherr1[0] = 10;
		noise_freq_table.freq_for_fherr1[1] = 12;
		noise_freq_table.freq_for_fherr1[2] = 18;
		noise_freq_table.freq_for_fherr1[3] = 40;
		noise_freq_table.freq_for_fherr1[4] = 72;
